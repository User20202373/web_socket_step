package com.tenco.web_socket_step.stpmp;

import com.tenco.web_socket_step.stpmp.StompChatService;
...
public class StompController {
    ...
    public String index(@PathVariable("roomId") Long roomId, Model model) {
        ...
        model.addAttribute("roomId", roomId); // 프론트엔드에서 방 번호를 쓸 수 있도록 전달
        model.addAttribute("chatList", stompChatService.findMessageByRoomId(roomId)); // 프론트엔드에서 방 번호를 쓸 수 있도록 전달
        return "stomp/index";
    }
    ...
    // SEND
    // destination: /pub/chat/message/5 <-- 끝 5를 낚아 챔
    // content-length: 41
