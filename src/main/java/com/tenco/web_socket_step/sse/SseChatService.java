...
public class SseChatService {
    ...
    // 접속한 클라이언트드을 보관할 자료 구조 -> ConcurrentHashMap 멀티 스레드에서 안전하다
    ...
    public SseEmitter createConnection(String clientId){
        ...
        // 필수 콜백 함수 등록 처리
        //연결 종료 / 타입아웃시 Map 목룍에서 제거 처리 (불필요한 메모리 누수 방지)
        ...
        // 1초 뒤에 : 서버가 클라이언트 연결을 끊어 버리고 있다.
        // 참고 : 0L 로 설정하면 무한으로 계속 연결이 되지만 실무에서는 좀비 커넥션이 생성이
